...
if fl is not None:
    ...
    os.chdir(r"C:\Users\PKamble\Downloads\0PRK")
...
if not region:
    ...
    df3 = df[df["City"].isin(state)]

# Filter the data based on regions, states and city.
...
if not region and not state and not city:
    ...
    filtered_df = df3[df["State"].isin(state) & df3["State"].isin(city)]
elif region and city:
    filtered_df = df3[df["State"].isin(region) & df3["City"].isin(city)]
elif region and state:
    filtered_df = df3[df["State"].isin(region) & df3["State"].isin(city)]
elif city:
...
##with cl2:
##    with st.extender("Region_ViewData"):
##        region = filtered_df.groupby(bt="Region",as_index=False)["Sales"].sum()
